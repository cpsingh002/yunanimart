         ...
         <div class="py-5">
             <div class="row g-4 align-items-center">
                 <div class="col">
                     ...
                     <h1 class="h3 m-0">Disease</h1>
                 </div>
                 <div class="col-auto d-flex">
                    <button type="button"  class="btn btn-primary" data-bs-toggle="modal"  data-bs-target="#exampleModalScrollable"> Add New Disease</button>
                 </div>
             </div>
         </div>
     </div>
                     ...
                     <table class="sa-datatables-init" data-order="[[ 1, &quot;asc&quot; ]]" data-sa-search-input="#table-search" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                         ...
                         <tbody>
                             ...
                             <tr>
                                 ...
                                 <td>{{$disease->name}}</td>
                                 <td>
                                    <div class="badge badge-sa-success">{{$disease->slug}}</div>
                                 </td>
                                 <td><img src="{{asset('admin/disea')}}/{{$disease->image}}" width="60"></td>
                                 <td>
                                     {{$disease->bodypart_id}}
                                 </td>
                                 
                                 <td>@if($disease->status == 1)<a href="#" onclick="confirm('Are you sure, You want to Deactive this disease') || event.stopImmediatePropagation()" wire:click.prevent='DeactivePart({{$disease->id}})'> Active </a> 
                    ...
                    <form class="form-horizontal" wire:submit.prevent="storeBodyPart">
                        ...
                        <div class="md-4">
                            <div class="form-group">
                                ...
                                <div class="col-md-12">
                                    <select class="form-control" wire:model="bodypart_id">
                                        <option value="">Select Body Part</option>
                                        @foreach($bodyparts as $bodypart)
                                            <option value="{{$bodypart->id}}">{{$bodypart->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('bodypart_id') <p class="text-danger">{{$message}}</p> @enderror
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-md-4 control-label">Disease Icon</label>
                            <div class="col-md-4">
                                <input type="file" placeholder="Category Icon" class="form-control input-md"  wire:model="image"/>
                                @if($image)
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" wire:submit.prevent="editPostData">
                        ...
                        <div class="form-group">
                            <label class="col-md-4 control-label">Disease Image</label>
                            <div class="col-md-4">
                                <input type="file" placeholder="Disease Icon" class="form-control input-md"  wire:model="newimage"/>
                                @if($newimage)
